...
long int* parallel_matrix_multiplication(long int* A, long int* B, 
                                            int matrix_size, 
                                            int num_of_threads);
void worker(int id, int start, int matrix_size,
...
long int* parallel_matrix_multiplication(long int* A, long int* B, 
    ...
    // initialize threads
    while ( index < N ) {
        threads.emplace_back(std::thread(worker, thread_id, index, matrix_size, A, B, C, num_of_threads));
        index += interval;
        thread_id++;
    }
    ...
    for ( int i = 0; i < num_of_rows; i++ ) {
        for ( int j = 0; j < num_of_columns; j++ ) {
            for ( int k = 0; k < chunk_size; k++ ) {
                ...
                int index_of_B = (k + start % matrix_size) * matrix_size + j;
